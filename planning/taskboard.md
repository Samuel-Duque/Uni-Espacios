...
| **EPIC-06** | Ciclo de Vida de Reservas y Bandeja de Aprobaciones | 5 | 📋 BACKLOG |
| **EPIC-07** | Control de Inventario y Verificaciones (Check-In / Check-Out) | 5 | 📋 BACKLOG |
| **EPIC-08** | Gestión de Calendario Académico y Clases Fijas | 4 | 📋 BACKLOG |
| **EPIC-09** | Pruebas de Carga, Concurrencia y Despliegue | 4 | 📋 BACKLOG |
...
| ID | Tarea | Prioridad | Dependencias | Estado | Criterios de Aceptación |
| :--- | :--- | :---: | :---: | :---: | :--- |
| **TSK-301** | Implementar `AuthModule` en NestJS con bcrypt y JWT | Alta | TSK-202 | 📋 BACKLOG | Registro y login validando obligatoriamente el dominio institucional `@elpoli.edu.co`. |
| **TSK-302** | Implementar `JwtAuthGuard` y decorador `@CurrentUser` | Alta | TSK-301 | 📋 BACKLOG | Guard extrae y valida token Bearer inyectando el payload del usuario en los controladores. |
| **TSK-303** | Implementar `RolesGuard` y decorador `@Roles(...)` | Alta | TSK-302 | 📋 BACKLOG | Endpoints protegidos devuelven 403 Forbidden si el rol no coincide con los permisos requeridos. |
...
| ID | Tarea | Prioridad | Dependencias | Estado | Criterios de Aceptación |
| :--- | :--- | :---: | :---: | :---: | :--- |
| **TSK-401** | Desarrollar `DisponibilidadService` para colisión con Clases Fijas | Crítica | TSK-202 | 📋 BACKLOG | Detecta si un horario colisiona con clases recurrentes considerando únicamente periodos académicos en estado `ACTIVO`. |
| **TSK-402** | Desarrollar servicio de consulta de colisión con Reservas Existentes | Crítica | TSK-202 | 📋 BACKLOG | Detecta cruces temporales con reservas existentes en estado `APROBADA` o `EN_USO`. |
| **TSK-403** | Crear motor unificado transaccional anti-solapamiento | Crítica | TSK-401, TSK-402 | 📋 BACKLOG | Ejecución atómica en `prisma.$transaction` con nivel de aislamiento serializable para evitar condiciones de carrera. |
| **TSK-404** | Endpoint de consulta de disponibilidad de espacio por fecha | Alta | TSK-403 | 📋 BACKLOG | `GET /api/espacios/:id/disponibilidad?fecha=YYYY-MM-DD` devuelve franjas libres y ocupadas. |
...
| ID | Tarea | Prioridad | Dependencias | Estado | Criterios de Aceptación |
| :--- | :--- | :---: | :---: | :---: | :--- |
| **TSK-501** | Endpoints de consulta de Sedes, Bloques y Espacios con filtros | Alta | TSK-202 | 📋 BACKLOG | `GET /api/espacios` permite filtrar por `sedeId`, `bloqueId`, `tipo`, `capacidadMin` y categoría de implementos. |
...
| ID | Tarea | Prioridad | Dependencias | Estado | Criterios de Aceptación |
| :--- | :--- | :---: | :---: | :---: | :--- |
| **TSK-601** | Endpoint para creación de solicitud de reserva | Crítica | TSK-403, TSK-302 | 📋 BACKLOG | `POST /api/reservas` valida disponibilidad, crea reserva en `PENDIENTE` y registra auditoría. |
...
| **TSK-604** | Endpoints de aprobación/rechazo para `GESTOR_ESPACIO` | Crítica | TSK-403, TSK-303 | 📋 BACKLOG | `PATCH /api/reservas/:id/estado` exige motivo en rechazos y ejecuta transacción atómica. |
...
| ID | Tarea | Prioridad | Dependencias | Estado | Criterios de Aceptación |
| :--- | :--- | :---: | :---: | :---: | :--- |
| **TSK-701** | CRUD de Inventario de Implementos por Espacio en NestJS | Alta | TSK-202, TSK-303 | 📋 BACKLOG | Endpoints `GET /api/espacios/:id/inventario`, `POST /api/inventario`, `PATCH /api/inventario/:id` protegidos por rol. |
| **TSK-702** | Endpoint para registrar Verificación de Check-In | Crítica | TSK-701, TSK-601 | 📋 BACKLOG | `POST /api/reservas/:id/check-in` valida ventana horaria, registra estado de cada implemento y cambia reserva a `EN_USO`. |
| **TSK-703** | Endpoint para registrar Verificación de Check-Out y Novedades | Crítica | TSK-702 | 📋 BACKLOG | `POST /api/reservas/:id/check-out` evalúa faltantes/daños, cambia reserva a `FINALIZADA` y genera alerta si hay novedad. |
...
| ID | Tarea | Prioridad | Dependencias | Estado | Criterios de Aceptación |
| :--- | :--- | :---: | :---: | :---: | :--- |
| **TSK-801** | CRUD de Periodos Académicos (`SUPERADMIN`) | Media | TSK-202, TSK-303 | 📋 BACKLOG | Endpoints para crear, activar y cerrar semestres académicos con validación de fechas. |
| **TSK-802** | CRUD y carga masiva de Clases Fijas por Espacio | Alta | TSK-801 | 📋 BACKLOG | Registro de horarios semanales recurrentes asignados a un periodo académico específico. |
...
| **TSK-804** | Validaciones cruzadas de integridad temporal en clases fijas | Media | TSK-802 | 📋 BACKLOG | Impide registrar clases con horas invertidas o fuera de los límites del periodo académico. |
...
| ID | Tarea | Prioridad | Dependencias | Estado | Criterios de Aceptación |
| :--- | :--- | :---: | :---: | :---: | :--- |
| **TSK-901** | Pruebas unitarias y de integración del motor de solapamientos e inventario | Crítica | TSK-403, TSK-703 | 📋 BACKLOG | Suite de pruebas en NestJS con Jest/Vitest cubriendo solapamientos y actas de verificación. |
